...
class AdminAuthController extends Controller
{
    ...
    public function showProfile($faculty_ID)
    {
        ...
        return view('admin.profile', compact('faculty'));
   } else{
    return redirect()->back()->withErrors('error','UnAuthorized Attempt to access Admin Portal !');
    }}
    public function login(Request $request)
    {
        try {
            // Access the submitted form data from the request directly
            $credentials = session('admin_credentials', $request->all());
            ...
            if (!$faculty || $faculty->ROLE != 'admin') {
                ...
                return redirect()->back()
                    ->withInput()
                    ->withErrors(['message' => 'No Admin member found with these credentials.']);
            }

            // Manually attempt login using the faculty's data
            Auth::guard('admin')->login($faculty);

            // Store faculty data in session
            session(['admin_data' => $faculty]);


            // Redirect to faculty profile view
            return redirect()->route('admin.profile', ['faculty_ID' => $faculty->Faculty_ID]);
...
public function destroy($ID)
{
    ...
    return redirect()->back()->with('success', 'The concerned member dismissed as an admin successfully');
}
